...
The single most important thing this file does is tell you **where a given change belongs**. Decide which project owns the change *first*, then follow that project's local rules. Cross-cutting changes flow in one direction: **API → OpenAPI contract → `libs/api-types` → `apps/web`.**
...
│
├── apps/
│   ├── api/                    # .NET 10 Web.Api host (apps/api/Api.csproj) — composition root  (Nx project: "api")
...
> **Convention:** every .NET **class library** lives under `libs/` (one folder per project); only the **Web.Api host** (`apps/api/Api.csproj`) lives under `apps/api`. `share.slnx` references all of them.
...
**Golden rule of the monorepo:** never create a runtime dependency from `apps/web` into `apps/api`'s source, or vice-versa. The *only* thing they share is the generated contract in `libs/api-types`. If you find yourself wanting to import across that boundary directly, the answer is almost always "extend the OpenAPI contract and regenerate types."
...
- Anything hand-written that *augments* the generated types (helpers, narrowed aliases) goes in `libs/api-types/src/lib/api-types.ts` and is re-exported from `index.ts`.
- The path alias `@share/api-types` is defined in `tsconfig.base.json`. Import from the alias, never via a relative `../../libs/...` path.
- **If the Web.Api host's location or port changes, update the generation commands** (`gen:types`, `gen:types:local`, `serve:api` in `package.json`, the `serve` target in `apps/api/project.json`, and the OpenAPI output path) so the pipeline keeps working.
...
- `Application` depends on `Domain` + `SharedKernel`. **It must NOT reference `Infrastructure` or `Web.Api`.**
- `Infrastructure` depends on `Application` (and inward). It implements abstractions the Application defines.
- `Web.Api` is the composition root — it references `Infrastructure` and wires everything together at startup.
...
The class-library layers live under `libs/` (one folder per project); the Web.Api host is `apps/api/Api.csproj`; tests live under `apps/api/tests/`.
...
| `Web.Api` (`apps/api/Api.csproj`) | Presentation / composition root | Minimal-API endpoints, middleware, exception handler, DI wiring, OpenAPI, request mapping. **Emits the OpenAPI document** consumed by `libs/api-types`. |
...
- Interface lives in `Application/Abstractions/...` (e.g. `IApplicationDbContext`, `IUserContext`, `ITokenProvider`, `IPasswordHasher`).
...
The matching endpoint lives in `apps/api/Endpoints/<Feature>/<UseCase>.cs` (the Web.Api host).
...
- One endpoint per file under `Web.Api/Endpoints/<Feature>/<UseCase>.cs`, `internal sealed`.
...
dotnet run --project apps/api                  # run the Web.Api host directly
...
5. **Web.Api** (`apps/api/Endpoints/<Feature>/`): add the `IEndpoint`, map request → command/query, call the handler, return `result.Match(...)`.
